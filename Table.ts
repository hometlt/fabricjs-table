...
class FabricTable extends fabric.Group {
  ...
  constructor(o: Partial<fabric.TableOptions>) {
    ...
    let options = Object.assign({},this.getDefaultProperties()) as fabric.TableOptions
  ...
  insertColumn(position = -1, width = this._cols[this._cols.length - 1].width || 1): void {
    ...
    this.width! += width;
    this._cols.splice(position, 0, {index: position, width} as fabric.TableColumn);
  ...
  insertRow(position = -1, height = this._rows[this._rows.length - 1].height || 1): void {
    ...
    this.height! += height;
    this._rows.splice(position, 0, {index: position, height} as fabric.TableRow);
    let expandedCells: fabric.TableCell[] = [];
  ...
  private _createCell(x: number, y: number, cell: fabric.TableCellOptions = {}) {
    ...
    if(!this._rows[y]){
      this._rows.push({ index:y, height: this.minRowHeight } as fabric.TableRow)
    }
    if(!this._cols[x]){
      this._cols.push({index:x, width: this.minColumnWidth } as fabric.TableColumn)
    }
    ...
    for (let xi = 0; xi < w; xi++) {
      for (let yi = 0; yi < h; yi++) {
        ...
        if(!this._rows[y + yi]){
          this._rows.push({index:y + yi, height: this.minRowHeight } as fabric.TableRow)
        }
        if(!this._cols[x + xi]){
          this._cols.push({index:x + xi, width: this.minColumnWidth } as fabric.TableColumn)
        }
...
declare module "fabric" {
  export namespace fabric {
    ...
    interface IObservable<T> {
      ...
      on(eventName: "selection:begin", handler: (e: CellsSelectionEvent) => void): T;
      ...
      on(eventName: "object:selection:begin", handler: (e: CellsSelectionEvent) => void): T;
      on(eventName: "object:selection:end", handler: (e: CellsSelectionEvent) => void): T;
      on(eventName: "object:selection", handler: (e: CellsSelectionEvent) => void): T;
    ...
    class Table extends FabricTable {
      //todo history mixin
      history: { disabled?: boolean, undo: fabric.IObjectOptions[], redo: fabric.IObjectOptions[], state: fabric.IObjectOptions }
      disableHistory(): void;
      enableHistory(): void;
      addUndoState(): void;
      initUndo(): void;
